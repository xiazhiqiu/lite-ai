/**
 * 会话存储的**文件实现**（SessionStore 的默认后端）。
 *
 * 逻辑**逐行搬自** `session.ts` 的既有实现——这是行为保持的重构：
 * 落盘位置、事件信封格式、parentUuid 链、snip/compact 边界语义全部不变。
 * 验收标准 = `test/session.test.ts` 的 36 个既有用例零改动全绿。
 */
...
function unwrapMessage(event: SessionEvent): ChatMessage | null {
  if (event.message) {
    return { ...event.message, id: event.uuid } as ChatMessage
  }
  return null
}
...
class FileSessionStore implements SessionStore {
  ...
  async load(cwd: string, sessionId: string): Promise<ChatMessage[] | null> {
    try {
      const lines = await readLines(sessionFilePath(cwd, sessionId))
  ...
  async loadTranscript(cwd: string, sessionId: string): Promise<PersistedTranscriptEntry[] | null> {
    try {
      const lines = await readLines(sessionFilePath(cwd, sessionId))
  ...
  async appendContextCollapseSpan(cwd: string, sessionId: string, span: CollapseSpan): Promise<void> {
    const dir = projectDir(cwd)
    const filePath = sessionFilePath(cwd, sessionId)
    await mkdir(dir, { recursive: true })
  ...
  async loadContextCollapseState(cwd: string, sessionId: string): Promise<ContextCollapseState | null> {
    try {
      const lines = await readLines(sessionFilePath(cwd, sessionId))
